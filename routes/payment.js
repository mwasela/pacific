...
//manual endpoint to trigger STK push for unpaid visits
router.post('/unpaid/stk', getAccessToken, async (req, res) => {
    const { visit_id, phone_no } = req.body;

    let phone_number = phone_no;

    //phone regex validation and formatting
    if (!phone_number) {
        return res.status(400).json({ error: "phone_no is required" });
    }
    ...
    try {
        ...
        console.error("Error triggering STK Push for unpaid visit:", error);
        res.status(500).json({ error: "Failed to trigger STK Push for unpaid visit." });
    }
});
